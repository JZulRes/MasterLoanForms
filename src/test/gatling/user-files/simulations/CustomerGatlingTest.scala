...
class CustomerGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Customer entity")
        ...
        .repeat(2) {
            ...
            .get("/masterloanforms/api/customers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new customer")
            .post("/masterloanforms/api/customers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "cedulaCustomer":null
                , "typeIdCustomer":"SAMPLE_TEXT"
                , "signatureWallet":"SAMPLE_TEXT"
                }""")).asJSON
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/masterloanforms${new_customer_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created customer")
            .delete("/masterloanforms${new_customer_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
